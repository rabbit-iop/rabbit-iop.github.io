...
线上地址：https://zhoushugang.gitee.io/
...
> amfe-flexible 手淘前端团队   做移动端rem基准值的动态设置   320px -> 32  375px -> 37.5
...
补充：书写css代码，让px转换rem单位，使用一个vscode插件  cssrem

- vscode 插件搜索 cssrem  进行安装即可

- 需要在 设置中  cssrem换算的时候使用80的比例（1920的设计稿量取尺寸），基准值是24px
  ...
  加上配置在 json 文件中保存，重新启动vscode即可生效。
...
- row 容器
  - 行高 1.05  内间距  12px 36px   字体大小 12px
  - .icon-dot 字体图标   绝对定位  左边0.64rem  透明度0
  - 鼠标经过后：背景 rgba(255, 255, 255, 0.1)  透明度1  颜色 #68d8ff
- col容器
...
第三步：按照需求定制  https://www.echartsjs.com/zh/option.html#title
...
- 柱子颜色，和工具提示。
  ...
  <style>
    .box{
      ...
      height: 300px;
      border-radius: 50%;
      /* 线性渐变三要素：渐变方向  起始颜色  结束颜色 */
      /* 0deg 从下到上渐变  正角度是顺时针 to (top left bottom right)  渐变方向 */
      /* background: linear-gradient(to top,red,green); */
      /* 径向渐变四要素：中心点(可用坐标)  半径  起始颜色 结束颜色 */
      /* 半径 at 中心，起始颜色，结束颜色 */
      background: radial-gradient(150px at left top,red,green);
    }
    </style>
  ```
